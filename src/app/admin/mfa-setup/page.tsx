...
export default function AdminMfaSetupPage() {
  ...
  useEffect(() => {
    if (recovered) {
      setNotice(
        "Your recovery code was accepted. For safety, your previous authenticator was reset. Scan the new QR code below to secure this admin account again."
      );
    }
  }, [recovered]);
  ...
  const loadRecoveryStatus = async () => {
    try {
      const response = await authorizedFetch("/api/admin/mfa/recovery-code");
  ...
  const initializePage = async () => {
    ...
    try {
      ...
      setError("An unexpected error occurred while loading 2FA setup.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDisableMfa = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await authorizedFetch("/api/admin/mfa/recovery-code", { method: "DELETE" });
      } catch (cleanupError) {
            ...
            <div className="flex min-w-0 items-start gap-3.5">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                  Protect admin access with an authenticator app and a one-time
                ...
                <div className="mt-4 space-y-3">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <p className="mt-1 text-sm font-semibold text-slate-900">
                      {verifiedFactor.friendly_name || "MatchIndeed Admin"}
                    </p>
                  </div>
